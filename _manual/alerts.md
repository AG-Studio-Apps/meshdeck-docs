...
**Deploy stackGuard.** Under **Agent**, tap **Deploy stackGuard**. meshDeck runs one small container on the host that watches the Docker socket, **read-only**, with every Linux capability dropped and no inbound port. It never starts, stops or changes anything — the app does that over its own connection. Be clear-eyed about the grant: **anything that can talk to the Docker socket can control the host**, so deploy it only on hosts you already trust meshDeck with. Once it is running the host reports in every couple of minutes, and real alerts start arriving.
