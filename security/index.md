...
- **What the relay stores.** Your Apple push token; random identifiers and hashed secrets (SHA-256, so a copy of its file is not a set of working credentials); your per-kind preferences; and for each enrolled host, which of your phones are attached and when it last checked in. No names, no addresses, no alert text.
