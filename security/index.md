...
- **Your push token is stored as it is**, because the relay has to hand it to Apple to deliver a notification. A push token is not a secret, and the relay does not treat it as one: to keep the identity that is attached to your hosts, an app must prove it with the private credential the relay issued when it first registered. A repeat token on its own gets a fresh, unattached identity and cannot touch an existing one — so it cannot take over a registration, mute alerts or remove hosts. And no alert can be read off the relay in any case, because the key that decrypts one never leaves your phone and the host.
